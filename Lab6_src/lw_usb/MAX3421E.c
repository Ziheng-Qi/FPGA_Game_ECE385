...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE send_buffer[2];
	BYTE recv_buffer[2];
	// Set up my send buffer!!!
	send_buffer[0] = reg | 0x02;  // Set wr bit
	send_buffer[1] = val;         // Data to write

	// Select MAX3421E (my only slave, so slave 0)
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);

	// See xcpi.c. This function requires: XSpi pointer, send buffer, receive buffer, byte count
	// So we include Instance, send 2 bytes and receive 2 bytes
	Status = XSpi_Transfer(&SpiInstance, send_buffer, recv_buffer, 2);

	// printf if error occurs
	if (Status != XST_SUCCESS) {
		xil_printf("SPI device failed to write register 0x%02X, Status: %d\n", Status);
	}
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	// Initialize send and receive buffers
	BYTE send_buffer[nbytes + 1];  // +1 used to skip to register address
	BYTE recv_buffer[nbytes + 1];
	// Set up my send buffer!!!
	send_buffer[0] = reg | 0x02;  // Set wr bit
	...
	// See xcpi.c. This function requires: XSpi pointer, send buffer, receive buffer, byte count
	// So we include Instance, send 2 bytes and receive 2 bytes
	Status = XSpi_Transfer(&SpiInstance, send_buffer, recv_buffer, nbytes + 1);

	// printf if error occurs
	if (Status != XST_SUCCESS) {
		xil_printf("SPI device failed to write %d bytes to register 0x%02X, Status: %d\n",
				   Status);
	}
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE send_buffer[2];
	BYTE recv_buffer[2];
	send_buffer[0] = reg;      // 0 for read and it should be 0 here already
	send_buffer[1] = 0x00;
	...
	// printf if error occurs
	if (Status != XST_SUCCESS) {
		xil_printf("SPI device failed to read register 0x%02X, Status: %d\n", Status);
		return 0;
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	// printf if error occurs
	if (Status != XST_SUCCESS) {
		xil_printf("SPI device failed to read %d bytes from register 0x%02X, Status: %d\n", Status);
		return data;
	}

	for (int i = 0; i < nbytes; i++) {
		data[i] = recv_buffer[i + 1];
	}
	return (data + nbytes);
}
