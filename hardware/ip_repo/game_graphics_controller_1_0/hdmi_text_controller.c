...
{
	if (color > 15) return;
	uint32_t rgb_value = ((red & 0xF) << 8) | ((green & 0xF) << 4) | (blue & 0xF);

	uint8_t word_index = color / 2;
	uint8_t is_high = color % 2;

	if (is_high) {
		hdmi_ctrl->PALETTE[word_index] = (hdmi_ctrl->PALETTE[word_index] & 0x0000FFFF) | (rgb_value << 16);
	} else {
		hdmi_ctrl->PALETTE[word_index] = (hdmi_ctrl->PALETTE[word_index] & 0xFFFF0000) | rgb_value;
	}
}
